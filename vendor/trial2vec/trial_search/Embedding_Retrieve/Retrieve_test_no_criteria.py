...
# 5. 修改模型内部配置 - 删除criteria
original_ctx_fields = copy.deepcopy(model.config['ctx_fields'])
modified_ctx_fields = [field for field in original_ctx_fields if field != "criteria"]

# 更新模型内部配置
model.config['ctx_fields'] = modified_ctx_fields
model.model.config['ctx_fields'] = modified_ctx_fields  # 更新模型的BuildModel配置
...
inputs = {
    'x': query_row,
    'fields': fields,
    'ctx_fields': modified_ctx_fields,  # 使用修改后的ctx_fields
    'tag': 'nct_id'
}
