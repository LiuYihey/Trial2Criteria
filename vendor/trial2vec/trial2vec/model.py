...
class Trial2Vec(TrialSearchBase):
    ...
    def load_model(self, checkpoint):
        ...
        state_dict = torch.load(checkpoint_filename, weights_only=False)

        if config_filename is not None:
            config = self._load_model_config(config_filename)
            # 保留保护原始bert_name的逻辑
            original_bert_name = self.config.get('bert_name')
            self.config.update(config)
            if original_bert_name:
                self.config['bert_name'] = original_bert_name
                print(f"--- [INFO] Preserved original bert_name: {original_bert_name} ---")
    ...
    def _build_val_docs(self, train_data, val_data):
        ...
        # 修复DataFrame.applymap已弃用的警告
        # 将 df.applymap(str) 改为 df.map(lambda x: x.map(str) if hasattr(x, 'map') else str(x))
        df_va = df_va.map(lambda x: x.map(str) if hasattr(x, 'map') else str(x))
    ...
    def _input_data_check(self, inputs):
        ...
        try:
            # 修复DataFrame.applymap已弃用的警告
            # 将 df.applymap(str) 改为 df.map(lambda x: x.map(str) if hasattr(x, 'map') else str(x))
            _ = df.map(lambda x: x.map(str) if hasattr(x, 'map') else str(x))
        except:
