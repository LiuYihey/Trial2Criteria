...
emb_dict = torch.load('Trial2Vec-main/Phase3_emb_only_fields.pth', weights_only=False)['emb']
all_ids = list(emb_dict.keys())
all_embs = list(emb_dict.values())

# 使用相对路径
raw_df = pd.read_csv('Retrieval_Base/RAG_base_phase3.csv')
raw_df = raw_df.set_index('nct_id')

# 2. 加载 Trial2Vec 模型和权重
# model = Trial2Vec()
model = Trial2Vec(bert_name='Trial2Vec-main/pretrained_models/Bio_ClinicalBERT')
model.from_pretrained('Trial2Vec-main/trial_search/pretrained_trial2vec')

# 3. 读取 demo case
case_df = pd.read_csv('Retrieval_Base/RAG_base_phase3.csv')
...
# 4. 获取并显示原始配置
with open('Trial2Vec-main/trial_search/pretrained_trial2vec/model_config.json', 'r') as f:
    config = json.load(f)
...
print(f"原始 ctx_fields: {ctx_fields}")

# 5. 修改模型内部配置 - 完全移除ctx_fields
# 保存一份原始配置用于对比
original_ctx_fields = copy.deepcopy(model.config['ctx_fields'])

# 更新模型内部配置 - 完全移除ctx_fields
model.config['ctx_fields'] = []
model.model.config['ctx_fields'] = []  # 更新模型的BuildModel配置
...
# 6. 对 case 进行 encoding
inputs = {
    'x': query_row,
    'fields': fields,
    'ctx_fields': [],  # 使用空的ctx_fields
    'tag': 'nct_id'
}
